...
# Named colors with display names — shown as swatches in label manager
LABEL_COLOR_OPTIONS = {
    "Red":    "#e74c3c",
    "Orange": "#e67e22",
    "Yellow": "#f1c40f",
...
def label_manager(user_id: str):
    """Dedicated section for creating and managing labels."""
    st.markdown("### 🏷️ Labels")
    labels = get_labels(user_id)

    # Show existing labels with edit + delete
    if labels:
        for lbl in labels:
            ...
            current_color_name = COLOR_NAME_MAP.get(lbl["color"], color_names[0])
            ...
            new_color = LABEL_COLOR_OPTIONS[new_color_name]

            if c3.button("Save", key=f"lbl_save_{lbl['id']}"):
                update_label(lbl["id"], user_id, lbl["name"], new_color)
                st.rerun()
    ...
    new_name = nc1.text_input("Label name", placeholder="e.g. Business, Family, Tax",
                               key="new_lbl_name", label_visibility="collapsed")
    color_names = list(LABEL_COLOR_OPTIONS.keys())
    chosen_color_name = nc2.selectbox(
        "Color", color_names, key="new_lbl_color",
        label_visibility="collapsed",
        format_func=lambda n: f"● {n}",
    )
    if nc3.button("Create", type="primary", key="create_lbl"):
        if new_name.strip():
            # Prevent duplicates
            existing_names = [l["name"].lower() for l in labels]
            if new_name.strip().lower() in existing_names:
                st.warning(f'Label "{new_name.strip()}" already exists.')
            else:
                add_label(user_id, new_name.strip(), LABEL_COLOR_OPTIONS[chosen_color_name])
...
def label_multiselect(key_prefix: str, labels: list, current_ids: list) -> list:
    """
    Renders a multiselect for labels. Returns list of selected label id strings.
    Shows color swatches via format_func on the options.
    """
    if not labels:
        st.caption("No labels yet — create them in the Label Manager below.")
        return []

    # Build options as label names (display), map back to id on selection
    name_to_id  = {l["name"]: str(l["id"]) for l in labels}
    id_to_label = {str(l["id"]): l for l in labels}

    # Pre-select based on current_ids
    default_names = [id_to_label[i]["name"] for i in current_ids if i in id_to_label]
    ...
    selected_names = st.multiselect(
        ...
        format_func=lambda n: f"● {n}",   # bullet gives a hint of color intent
...
def expense_detail_form(key_prefix, categories, labels, defaults=None):
    ...
    with st.expander("More details", expanded=bool(defaults)):
        ...
        selected_label_ids = label_multiselect(key_prefix, labels, current_ids)
