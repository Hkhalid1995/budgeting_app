...
def get_expenses_for_period(user_id, since_date: str):
    """
    Fetch all expenses on or after since_date (ISO format: YYYY-MM-DD).
    Joins budget_categories for name/icon/limit.
    """
    conn = get_conn()
    rows = conn.execute(
        """SELECT e.*, b.name as category_name, b.monthly_limit, b.icon
...
def get_spending_by_category_for_period(user_id, since_date: str):
    """
    Sum spending per category on or after since_date.
    Always returns all categories (spent=0 when no expenses in range).
    """
    conn = get_conn()
    rows = conn.execute(
        """SELECT b.id, b.name, b.monthly_limit, b.icon,
           ...
           LEFT JOIN expenses e
               ON e.category_id=b.id
               AND e.user_id=?
               AND e.date >= ?
           WHERE b.user_id=?
           GROUP BY b.id""",
        (user_id, since_date, user_id),
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]
