...
def render():
    ...
    if step == 1:
        ...
        income_str = st.text_input(
            ...
            help="You can use commas — e.g. 150,000"
        )
    ...
    # ── Step 3: Budget categories ────────────────────────────────
        ...
        st.caption(f"Set limits per category. Add custom ones too. Income: **{currency} {income:,.0f}**")
        ...
        st.divider()
        if remaining < 0:
            st.warning(f"⚠️ You've allocated {currency} {abs(remaining):,.0f} more than your income.")
